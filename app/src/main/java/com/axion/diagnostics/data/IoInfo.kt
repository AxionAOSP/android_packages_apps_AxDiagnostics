...
data class IoSnapshot(
    val disks: List<DiskStats>,
    val readRateKbps: Float,
    val writeRateKbps: Float,
    val ioPressure: IoPressure
)
    ...
    private fun readIoPressure(): IoPressure {
        ...
        for (line in lines) {
            ...
            if (avgs != null) {
                ...
                if (line.startsWith("some")) { someAvg10 = a10; someAvg60 = a60; someAvg300 = a300 }
                if (line.startsWith("full")) { fullAvg10 = a10; fullAvg60 = a60; fullAvg300 = a300 }
            }
        }
        return IoPressure(someAvg10, someAvg60, someAvg300, fullAvg10, fullAvg60, fullAvg300)
    }

    private fun isWholeBlockDevice(name: String): Boolean =
        wholeBlockDevicePatterns.any { it.matches(name) }
}
